...
public final class AccountHandler {
    ...
    {
        ArrayList<String> accountInfo = IO.getMultiStringInput("Login Form", new String[]{"Email","Username","Password"});
        String email = accountInfo.get(0);
      
       // Check for valid email
        if(!EmailController.getInstance().checkFormat(email)){
          IO.outputMessage("Invalid Email!");
          return;
        }
        ...
        switch(database.verifyRegistration(email,username))
        {
            case 0:
                ...
                IO.outputMessage("Account Creation Success");
                break;
            case 1:
                IO.outputMessage("Invalid Email");
                break;
            case 2:
                IO.outputMessage("Invalid Username");
                break;
        }
    }
    ...
    public static Account login()
    {
        ...
        ArrayList<String> accountInfo = IO.getMultiStringInput("Login Form", new String[]{"Username","Password"});
        String username = accountInfo.get(0);
        String password = accountInfo.get(1);
        int accountID = database.verifyLogin(username, password);
        if(accountID == -1)
        {
            IO.outputMessage("Login Failed");
            return null;
        }
        ...
        IO.outputMessage("Login Successful");
    ...
    {
        IO.outputMessage("Logout Successful");
        return null;
    }
